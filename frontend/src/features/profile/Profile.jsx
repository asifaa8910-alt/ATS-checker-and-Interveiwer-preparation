            ...
            <div className="px-6 pb-6 relative">
              ...
              <div className="pt-16 space-y-4">
                ...
                <div className="flex gap-3 pt-2">
                  {profile?.linkedin && (
                    <a href={profile.linkedin} target="_blank" rel="noreferrer" className="p-2 bg-muted text-primary rounded-lg hover:bg-muted/80 border border-border transition">
                      <Linkedin className="h-4 w-4" />
                    </a>
                  )}
                  {profile?.github && (
                    <a href={profile.github} target="_blank" rel="noreferrer" className="p-2 bg-muted text-foreground rounded-lg hover:bg-muted/80 border border-border transition">
                      <Github className="h-4 w-4" />
                    </a>
                  )}
                  {profile?.portfolio && (
                    <a href={profile.portfolio} target="_blank" rel="noreferrer" className="p-2 bg-muted text-primary rounded-lg hover:bg-muted/80 border border-border transition">
            ...
            {profile?.skills?.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2">
                {profile?.skills?.map((skill, idx) => (
                  <span key={idx} className="px-2.5 py-1 bg-muted text-foreground text-xs font-semibold rounded-lg border border-border capitalize">
                    {skill}
                  </span>
                ))}
              </div>
            ...
            {profile?.projects?.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {profile?.projects?.map((proj, idx) => (
                  <div key={idx} className="p-4 border border-border rounded-xl bg-muted/40 space-y-2 text-xs">
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div className="p-6 border rounded-2xl bg-card space-y-4">
            ...
            <div className="space-y-4">
              {experience.map((exp, idx) => (
                <div key={idx} className="p-4 border border-border rounded-xl bg-muted/20 space-y-3 relative text-xs">
                  <button
                    type="button"
                    onClick={() => handleRemoveExperience(idx)}
                    className="absolute top-4 right-4 text-destructive hover:bg-destructive/10 p-1.5 rounded-lg border border-destructive/20"
                  >
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="px-6 py-2.5 border border-border rounded-xl text-xs font-semibold hover:bg-muted bg-muted/40 text-foreground transition"
            >
              Cancel
            </button>
          </div>
