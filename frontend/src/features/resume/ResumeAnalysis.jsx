        ...
        <div className="space-y-6 lg:col-span-1">
          ...
          <div className="p-6 rounded-2xl border bg-card text-card-foreground shadow-sm space-y-4">
            ...
            <form onSubmit={handleUpload} className="space-y-4">
              ...
              <div className="border-2 border-dashed rounded-xl p-5 text-center hover:border-primary transition cursor-pointer relative bg-muted/40">
                <input
                  type="file"
                  accept=".pdf,.docx"
        ...
        <div className="lg:col-span-2 space-y-6">
          {!currentAnalysis ? (
            ...
            <div className="space-y-6 animate-fadeIn">
              ...
              {/* Score header */}
              <div className="p-6 rounded-2xl border border-border bg-card text-card-foreground shadow-sm grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
                
                {/* Dial */}
                <div className="flex flex-col items-center text-center">
                  <div className="relative flex items-center justify-center h-28 w-28 rounded-full border-8 border-muted bg-muted/20">
                    <span className="text-3xl font-black tracking-tight text-primary">{currentAnalysis.atsScore}%</span>
                  </div>
                ...
                <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    ...
                    return (
                      <div key={idx} className="space-y-1.5 p-2 bg-muted/40 rounded-xl border border-border">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Edu */}
                <div className="p-5 border border-border bg-card rounded-2xl space-y-2">
                  ...
                  {currentAnalysis.analysis?.educationMatches?.length === 0 ? (
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {currentAnalysis.analysis?.educationMatches?.map((edu, idx) => (
                        <span key={idx} className="px-2 py-0.5 bg-muted text-foreground text-[10px] font-semibold rounded-md border border-border capitalize">
                          {edu}
                        </span>
                      ))}
                    </div>
                  ...
                  {currentAnalysis.analysis?.certificationMatches?.length === 0 ? (
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {currentAnalysis.analysis?.certificationMatches?.map((cert, idx) => (
                        <span key={idx} className="px-2 py-0.5 bg-muted text-foreground text-[10px] font-semibold rounded-md border border-border">
                          {cert}
                        </span>
                      ))}
                    </div>
              ...
              <div className="p-6 rounded-2xl border border-border bg-card space-y-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {currentAnalysis.analysis?.missingActionVerbs?.map((verb, idx) => (
                    <span key={idx} className="px-2 py-1 bg-muted hover:bg-muted/80 text-foreground text-[10px] font-semibold rounded-lg border border-border capitalize">
                      +{verb}
                    </span>
                  ))}
                </div>
              ...
              <div className="p-6 rounded-2xl border border-border bg-card space-y-4">
                ...
                <div className="space-y-3">
                  {currentAnalysis.analysis?.suggestions?.map((item, idx) => (
                    <div key={idx} className="p-3 bg-muted/40 rounded-xl text-xs leading-relaxed flex gap-3 border border-border">
